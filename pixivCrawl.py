...
class ThroughArtWorkerId:
    ...
    def download_static_images(self, session, mkdir_static):
        ...
        if len(static_url) > 1:
            mkdir_static = create_directory(self.img_id)
...
class ThroughWorkerId:
    ...
    def preDownload(self):
        try:
            # 如果没有文件夹，那就创建一个
            mkdirs_workers = os.path.join(os.getcwd(), "workers_IMG", self.getWorkerName())
            os.makedirs(mkdirs_workers, exist_ok=True)
            ...
            with ThreadPoolExecutor(max_workers=os.cpu_count()) as executor:
                futures = [executor.submit(self.download_image_or_gif, img_id, mkdirs_workers) for img_id in listId]
                for future in as_completed(futures):
                    future.result()
                    self.app.update_progress_bar(1)  # 更新进度条
            logging.info(f"下载完成，共有{len(os.listdir(mkdirs_workers))}张图片~")
        except TypeError:
    ...
    def download_image_or_gif(self, img_id, mkdirs):
        ...
        if data['error']:
            ...
            self.download_ugoira(data, img_id, mkdirs)
    ...
    # 下载动图
    def download_ugoira(self, data, img_id, mkdirs):
        delays = [frame['delay'] for frame in data['body']['frames']]
        with ThreadPoolExecutor(max_workers=os.cpu_count()) as executor:
            futures = [executor.submit(self.download_gif, data['body']['originalSrc'], img_id, mkdirs, delays)]
            for future in as_completed(futures):
                future.result()

    def download_gif(self, url, img_id, mkdirGif, delays):
        file_path = os.path.join(mkdirGif, f"@{self.getWorkerName()} {img_id}.gif")
        response = requests.get(url, headers=self.headers, verify=False)
        request_if_error(response)
        zip_content = io.BytesIO(response.content)
